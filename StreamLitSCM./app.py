...
with st.sidebar:
        ...
        initial_ioh = st.number_input('Initial Inventory on Hand ', min_value=0.0 , value=55.0 , step = 1.0,
                                      format= '%.2f')
        sigma = st.number_input('Daily demand std.dev (units/day) ', min_value=0.0 , value=0.0 , step = 0.1,
                                      format= '%.2f')
        method = st.radio(
                'Ordering Method',
        ...
        method_key = 'order_leadtime' if method == 'Lead-Time Ordering' else 'order'
...
# Formatting of input paramters cards
st.markdown("""
<style>
...
with c6:
       quick_card('Demand of ', f"{sigma:,.2f}", 'units/day')
...
if st.session_state.has_run:
       ...
       st.info('Set your paramters on the left and click Run **Simulation**')
